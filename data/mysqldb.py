...
class MySQL:
    ...
    def add_new_user(self, user: UserRegis):
        ...
        mycursor.execute(
            'INSERT INTO user_role(user_id, role_id) VALUES (%s, %s)', (user_id, Conf.USER))
    ...
    def update_user(self, current_user_id: int, user_update: UserUpdate):
        ...
        mycursor.execute(
            'DELETE FROM user_category WHERE user_id = %s', (current_user_id,))
        self.mydb.commit()
    ...
    def get_roles_by_user_id(self, user_id: int):
        mycursor = self.mydb.cursor(dictionary=True)
        mycursor.execute(
            'SELECT * FROM user_role WHERE user_id = %s', (user_id,))
        ...
        if bool(role_id_list):
            mycursor.execute(
                'SELECT * FROM roles WHERE id IN (%s)', role_id_list)
    ...
    def get_categories_by_user_id(self, user_id: int):
        mycursor = self.mydb.cursor(dictionary=True)
        mycursor.execute(
            'SELECT * FROM user_category WHERE user_id = %s', (user_id,))
        ...
        if bool(category_id_list):
            ...
            mycursor.execute(
                'SELECT * FROM categories WHERE id IN {}'.format(category_id_list))
    ...
    def add_article_scores(self, session_id: int, scores: List[ScoreInsert]):
        ...
        for score in scores:
            ...
            sql = 'INSERT INTO scores (session_id, article_id, url, category, domain, score, audio_path, audio_path_en) VALUES (%s, %s, %s, %s, %s, %s, NULL, NULL)'
            value = (session_id, score.article_id, score.url,
                     score.category, score.domain, score.score)
            mycursor.execute(sql, value)
        self.mydb.commit()
    ...
    def add_new_session(self, created_time: datetime):
        mycursor = self.mydb.cursor()
        mycursor.execute(
            'INSERT INTO sessions(created_time, completed) VALUES (%s, %s)', (created_time, 0))
        self.mydb.commit()
    ...
    def get_valid_session_id(self):
        mycursor = self.mydb.cursor(dictionary=True)
        mycursor.execute(
            'SELECT MAX(id) FROM sessions WHERE completed = %s', (1,))
        result = mycursor.fetchone()
    ...
    def get_article_score_by_uuid_and_audio_not_null(self, uuid: str):
        mycursor = self.mydb.cursor(dictionary=True)
        mycursor.execute(
            'SELECT * FROM scores WHERE article_id = %s AND audio_path IS NOT NULL LIMIT 1', (uuid,))
        result = mycursor.fetchone()
        if result is not None:
            return Score(**result)
        else:
...
if __name__ == '__main__':
    ...
    data = mysql.fetch_articles_ranking(
        session_id=57, category='chinh-tri', limit=15)
    for i in data:
        print(i)
